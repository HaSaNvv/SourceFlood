...
token = "6910504132:AAEXTKi5ImpLjcDyRWOZFAbxR952evUpzg0" #توكنك
chat_id = "6590885543" #ايديك
...
async def claimer(client,username):
    result = await client(functions.channels.CreateChannelRequest(title=f'[ {username} ]',about=f'Source - @FFFZK',megagroup=False))
    try:
...
def callB(call):
    global flag
    if call.data == "type":
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="اختر النوع من احدى الازرار", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="رباعي", callback_data="th3"), InlineKeyboardButton(text="بوتات", callback_data="fi5")], [InlineKeyboardButton(text="رباعي حرفين", callback_data="f5"), InlineKeyboardButton(text="سداسي صيني", callback_data="se7")],[InlineKeyboardButton(text="خماسي حرفين عشوائي", callback_data="f5i")]]))
...
def reset(msg):
    if db.exists('session') is False:
    	bot.send_message(chat_id, text="اهلا بك عزيزي\nبوت متخصص لجلب خاصيات\n– – – – –\nمطوره : @ssuxs", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="اضف حساب",callback_data="add")]]))
    	return 
    bot.send_message(chat_id, text="اهلا بك عزيزي\nبوت متخصص لجلب خاصيات\n– – – – –\nمطوره : @ssuxs", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="فحص عشوائي", callback_data="start")],[InlineKeyboardButton(text="من ملف", callback_data="combo"),InlineKeyboardButton(text="تثبيت", callback_data="us1")],[InlineKeyboardButton(text="اختيار نوع", callback_data="type"),]]))
